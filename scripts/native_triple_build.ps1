# scripts/native_triple_build.ps1
# AXIOM Compiler Native Self-Hosting and Triple-Build Verification Loop
#
...
# 3. Compiling the self-hosted frontend (Stage 1) using Stage 0 via C backend
# 4. Verifying Stage 1 against Stage 0 reference output on the test corpus
# 5. Compiling Stage 2 using Stage 1 via the **AXIOM Native Backend & Self-Linker**
# 6. Compiling Stage 3 using Stage 2 via the **AXIOM Native Backend & Self-Linker**
# 7. Computing and matching SHA-256 hashes of Stage 2 and Stage 3 native binaries for bit-identical reproducible build verification
...
Write-Host "=== AXIOM Native Triple-Build Verification Loop ===" -ForegroundColor Cyan
...
# 4. Compile Stage 1 (self-hosted frontend) using Stage 0 via C backend
...
# 5. Run Verification Loop across Corpus to check frontend correctness
...
# 6. Compile Stage 2 using Stage 1 using the AXIOM Native Backend & Self-Linker
Write-Host "[Stage 2] Building Stage 2 compiler using Stage 1 via native backend (-self-link)..." -ForegroundColor Green
...
Write-Host "[Stage 2] Compiled successfully to bin/axc_stage2_native.exe" -ForegroundColor Green

# 7. Compile Stage 3 using Stage 2 using the AXIOM Native Backend & Self-Linker
Write-Host "[Stage 3] Building Stage 3 compiler using Stage 2 via native backend (-self-link)..." -ForegroundColor Green
...
Write-Host "[Stage 3] Compiled successfully to bin/axc_stage3_native.exe" -ForegroundColor Green

# 8. Reproducible Build Verification (SHA-256 matching)
...
Write-Host "Stage 2 Native SHA-256: $hash2" -ForegroundColor Cyan
Write-Host "Stage 3 Native SHA-256: $hash3" -ForegroundColor Cyan

if ($hash2 -eq $hash3) {
    Write-Host "SUCCESS: Stage 2 and Stage 3 native compiler binaries are 100% bit-identical!" -ForegroundColor Green
    Write-Host "AXIOM Self-Hosted Native Code Generation and Self-Linking is fully verified!" -ForegroundColor Green
    exit 0
} else {
    Write-Error "FAILURE: Stage 2 and Stage 3 native compiler binaries differ!"
    exit 1
}
