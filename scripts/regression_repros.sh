...
rows=(
  ...
  # a variant ctor's arg count is checked against the payload shape (a tuple payload
  # given two args used to SIGSEGV; four other mismatches were silently wrong)
  "t_ctorarity|reject|"
  ...
  "t_drop|exit|42"
  # RFC 0015 P3 return-path free (default-on): a drop-typed local live on a RETURN path
  # is now freed on that path too (not only at block fall-through), so both early()-calls
  # drop their `r` -> 2; take()'s RETURNED aggregate stays un-dropped (escape exclusion).
  # The off-behaviour (0, leak) is pinned bidirectionally by ctgc_free_check.sh.
  "t_ctgcretfree|exit|2"
...
for jt in "t_jumptable:226" "t_jumptable2:28" "t_jumptable3:27" "t_ifchainjt:53"; do
  jname="${jt%%:*}"; jwant="${jt##*:}"
...
for z in "t_negbiglitcmp:42" "t_tostr:88" "t_localarrnoinit:12" "t_localstructnoinit:12" "t_localtuplenoinit:12"; do
  zname="${z%%:*}"; zwant="${z##*:}"
