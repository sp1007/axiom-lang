...
**Để fix (RFC float-arithmetic, follow-up):** (a) lower_binary_expr chọn OP_FADD/FSUB/FMUL/FDIV khi node_type là f32/f64 (thay vì luôn OP_IADD…); (b) chèn OP_CAST int→float (cvtsi2sd) khi một toán hạng là int-var còn vế kia float (hoặc YÊU CẦU `as f64` tường minh + chỉ chọn float-op); (c) literal float/int hỗn hợp đã do RFC 0005 + promotion lo phần kiểu.

> **(b) VẪN MỞ — đo lại 2026-07-30, và phạm vi RỘNG HƠN mô tả cũ.** Không chỉ int-*var*: mọi VỊ TRÍ mà typecheck không lan hint float xuống đều để lại node type nguyên, nên không có hằng float nào cho air_builder vật chất hoá. Bốn ca đã đo (sau khi hằng-literal đã được sửa): `H(a: 3)` vào **field f64** (`bin/probe4/i1.ax` — AIR: `%2: t3 = iconst %3` rồi setfld f64), `argf(3)` vào **param f64** (`h4.ax`), `mm.m(3)` vào **param f64 của method** (`g6.ax` bit 1), `let c: f64 = 3 + 1` (**biểu thức** int → f64, `g5.ax` bit 64). **Bản sinh đôi f32 của cả bốn đều ĐÚNG** vì `typecheck.ax:5208` (và họ hàng) chỉ hint `TYPE_F32`, cố ý "scoped to F32 only" ⇒ lại đúng lớp defect "hai bản sao của một cơ chế trôi lệch". Sửa = lan hint / chèn coercion tại các VỊ TRÍ đó (typecheck), rủi ro fixpoint ≠ 0 ⇒ việc riêng, không gộp. Bốn số hàng 4/8/9/11 đã được ĐẶT CHỖ trong `bin/t_intlitfloatctx.ax`. Cần test + giữ fixpoint (compiler không dùng float nên an toàn).
...
- Float literal luôn f64 (NODE_FLOAT_LIT). ⚠️ **CÂU "`let x: f64 = 3` → 3 adopt f64 (RFC 0005) → OK" LÀ SAI KHI ĐƯỢC VIẾT RA (2026-06-26) — chưa từng được ĐO.** Đo ngày **2026-07-30**: `let a: f64 = 3` cho **0.0**. typecheck adopt f64 đúng, nhưng `air_builder.lower_int_lit` vẫn phát **OP_ICONST** với type_id 9/10 → hằng số vào vreg lớp GPR trong khi lệnh float đọc XMM alias → 0.0 (hoặc giá trị CŨ còn sót ⇒ **-O0 và -O1 lệch nhau**). ✅ **FIXED 2026-07-30** (commit này): lower_int_lit phát OP_FCONST qua `emit_float_const` dùng chung với lower_float_lit + **verifier §9 `verify_air_const_types`** (OP_ICONST không được mang type_id float, và ngược lại) nên lỗi này không thể im lặng lần nữa. Oracle `bin/t_intlitfloatctx.ax` (trước: exit 1; sau: 42 ở -O0/-O1/-O2). **Bài học:** một khẳng định hành vi chưa đo = một giả thiết, và nó đã được tin cậy suốt hơn một tháng.
