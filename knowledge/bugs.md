...
Cần soi: `x86_encode_modrm_rr` / hàm tính modrm (shift `<<6`/`<<3` + OR + đọc reg_hw_reg(phys)) — tìm vreg local (reg/rm/mod) bị spill reload thành 0, HOẶC shift CL-form clobber (như BUG#24). Repro: objdump bất kỳ obj stage3-compiled, đếm `48 89 00`. **stage2 vs stage3 differential** trên cùng input cho thấy stage3 collapse modrm.
