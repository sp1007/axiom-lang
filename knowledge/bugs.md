...
⚠️ **NHƯNG là CLASS #3 (phụ thuộc áp lực thanh ghi):** repro nhỏ `bin/tshift.ax` (`(3<<6)|((4&7)<<3)|(5&7)`) → **stage2 compile ĐÚNG = 229**. Vậy stage2 KHÔNG miscompile const-shift nói chung; chỉ miscompile shift TRONG x86_encode_modrm_rr (hàm lớn, nhiều biến sống → CL-form clobber thanh ghi giữ reg_field/rm_field/accumulator). KHÔNG repro được ở test nhỏ. → phải: (a) objdump `ax_x86_encode_modrm_rr` trong obj stage3-compiled (rebuild stage2→compiler giữ axiom_temp.obj, có symbol) so với stage1-obj, tìm chỗ CL-form shift clobber; HOẶC (b) marker-pin trong build thật. Fix gốc class: const_shift_amount/regalloc reserve RCX cho rm-field khi có shift (xem lịch sử BUG#24 v2 reserve RCX bị revert do tăng spill — cân nhắc spill-safe CL-form).
