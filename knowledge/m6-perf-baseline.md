...
NEXT_TARGET_2026_07_30c: "MEASUREMENT PROTOCOL, not codegen. Use scripts/perf_layout_dist.ps1 (median over N controlled layout offsets + reported spread) for every M6 number from now on; a delta below the spread is NOT a result. Next concrete step: give the NASM floors the same distribution treatment so the M6-codegen gate ratio is a median-over-layouts instead of one draw. DO NOT build a loop-header alignment pass — refuted 2026-07-30, see below."
RESOLVED_NEXT_TARGET_2026_07_30b: "(done) peephole 1f was RED on float parameter prologues. Cause was ABI ordering, NOT either suspected mechanism: the float spill scratch XMM2 is also win64's 3rd float arg register, and emit_param_prologue's snapshot-first phase is what protects it. Guard = def must not read OPND_PHYS. SHIPPED d454c49, B==C B71DF82A, 581/581. See bug-float-arg-reg-unprotected and session-handoff-2026-07-30c."
LAYOUT_VARIANCE_CORRECTION_2026_07_30: "The '~8-10% variance per run, measure paired and alternating' rule recorded throughout this file is MISATTRIBUTED. Within one binary, best-of-9 is repeatable to ~1%. The large term is DETERMINISTIC LAYOUT sensitivity, resampled whenever a binary is rebuilt — identical instruction streams shifted by multiples of 16 bytes measured 134.8 / 153.1 / 167.3 ms on one shape (24% spread). Pairing and alternating remove DRIFT but NOT layout bias: they compare the same two binaries repeatedly, so same-sign consistency across pairs is NOT evidence the effect belongs to the compiler change. Three past 'results' were this phantom — 1e 'cost fib 5.1%', xorshift '+4.6% from removing instructions', and 1f 'cost callloop 5.7-8.9% with 4/4 pairs same sign' (re-measured over layouts: medians 143.5 vs 143.5 ms, i.e. EXACTLY ZERO, with the fast/slow clusters merely swapped between compilers). Peephole 1f's tailrec win by contrast SURVIVES the distribution treatment: 21.4 -> 18.4 ms medians, -14.0%, and the two sample sets do not overlap at all (max 19.0 < min 21.0)."
