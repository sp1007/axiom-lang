...
- ✅ **Literal-pattern match on FLOAT/STRING scrutinee → REJECT** (`07139af`, A==B `3453BD82`, 460/460):
  found while probing the negative-match feature. `match x(f64): 2.0:` / `match s: "hi":` silently
  lowered to NOTHING (switch lowering is int/bool/char only) → function returned garbage RAX (a
  pre-existing accept-then-miscompile, BUG#53 class). Now rejected in the NODE_MATCH_ARM typechecker
  (typecheck.ax ~L2961, beside the non-sum-variant-pattern reject), gated to concrete primitive ids
...
- **`Self` type** — ✅ **Self-as-param CONFIRMED WORKING 2026-07-19c** (probe SP6/SP8/SP9 all →42): `other: Self` resolves to the receiver type in in-struct methods AND free-function methods AND combined with `-> Self` return + method chaining (`a.merge(b).merge(c)`). `-> Self` return + chaining already worked (`575a69a` [[self-type-return-fixed]]). **✅ INTERFACE VTABLE DYNAMIC DISPATCH — DONE (STALE note closed 2026-07-24)**: RFC 0029 shipped the full vtable-box mechanism long before this note was written. `ensure_iface_box_type`/`build_interface_value` (air_builder.ax:1678/1699) synthesize an inline per-value box `{data, m0..m(N-1)}` at every T→I coercion site; `iface.m()` lowers to `data=box.field0`, `fptr=box.field(slot+1)`, indirect `OP_CALL fptr(data, args...)` (air_builder.ax:1837–1887, comment "Closes BUG#71"). Verified robust 2026-07-24: `t_ifaceconsumer(46)` (std/log's `Box[LogSink]` pattern — interface value in a struct field, two impls dispatch to their own method), plus banked `t_ifacevecpoly(70)` (polymorphic dispatch through Vec[Interface] in a loop + mut-reassign to a different impl + 2-method vtable slot ordering), O0–O3 identical. ~15 iface oracles total incl. `t_ifaceglobal(86)` (interface globals also work now — the "clean-REJECT" note for those is stale too). Nothing open here. NOTE: untyped `self` (`fn m(self)`) requires the method be INSIDE a struct body (RFC 0002 `current_struct`); a top-level free-fn method must write `self: Type` — by design, not a bug.
- **Rewrite aspirational std modules** (iter/json/log/net/fmt/…) from out-of-grammar dialect to real grammar — see `std/MODULE_STATUS.md`; needs enum/Self. Backlog, not blocking.
  - **`to_str` number formatting** — ✅ **i64 DONE** `0be41f8` (`t_tostr(88)`), ✅ **f64 DONE** `b81293a` (overload by arg type, mirror `ax_sprintf_local`, 6-digit frac; `t_tostrf(91)`), ✅ **hex DONE** `afbf5e3` (`to_hex(i64)` lowercase; `t_tohex(123)`), ✅ **general radix DONE** `d92a6e4` (`to_radix(i64,base)` base 2..36, subsumes bin/oct/hex; `t_toradix(99)`). ✅ **numeric UFCS DONE** `796a7a8` (`n.to_str()`/`n.to_hex()`/`n.to_radix(b)`/`f.to_str()` — widened UFCS dispatch to ALL primitives; `t_numufcs(11)`). ✅ **numeric UFCS widening DONE** `a02db69` (narrow-int receiver i8/i16/i32/u8/u16/u32 → i64 free fn, gates-only no cast; `t_numufcswide(7)`; also closed misdiagnosed "generic-inst field mono Layer 2"). ✅ **u64 decimal DONE** `732963c` (`to_str(u64)` unsigned overload; `t_tostru64(19)`). ✅ **uppercase hex DONE** (STALE-note closed 2026-07-24): `to_hex_upper(i64)` already lives in std/string.ax:471 (mirrors `to_hex`, 'A'..'F'), oracle `t_tohexup(77)` wired at regression_repros.sh:190 and PASSES on driver, incl. numeric UFCS `n.to_hex_upper()`. Nothing to implement — the "STILL OPEN" note predated `485d02f`. Bigger fmt-module rewrite (json/log/iter still out-of-grammar) remains backlog. NOTE: free-fn overload by arg type CONFIRMED working (to_str i64+f64 coexist).
...
- ~~**Jump-table dispatch** optimization~~ — ✅ **RFC 0028 CLOSED 2026-07-22** (A==B `57738F36`, 485/485).
  `match` compare-tree shipped `d85ec48`; **§7e if/elif-chain recognition** shipped 2026-07-22
  (or-arms, named consts, unsigned scrutinees, threshold 8). **`-jumptable` stays OPT-IN by
  measured decision**: 1.38x (match) / 1.47x (if-chain) on dispatch-bound USER code, but
  **−0.8% (zero) on the compiler's own self-build** — the compiler's longest chains interleave
  `and`-guarded clauses that disqualify them, and self-build is not dispatch-bound. This also
  kills the §3–§7c O(1) `.rodata`+reloc variant. Full write-up incl. the two test-invisible bugs
  (silent no-fire via a shared scratch buffer; block leak on bail → 256MB allocator OOM) in
  [[rfc0028-closed-2026-07-22]].
- ~~**Vec/collection HOF methods**~~ — ✅ **SHIPPED**: map/filter/fold (`b956c01`), any/all/find (`d57de80`), count/position/take_while/skip_while/reverse (`f7ed1e6`), **enumerate `f63a130` / partition `a9cfd91` / zip `bced0d2`**. ✅ **B2 CLOSED 2026-07-22 — the "blocked on tuple×generic element" note was STALE**: all three had already shipped once `aa419a2` landed. Re-probed the full surface on driver `57738F36`, ALL correct and IDENTICAL across O0/O1/O2/O3: partition-with-real-lambda (62), enumerate T=str 16B-in-tuple (15), zip U≠T (21), empty-enumerate + zip length-truncation (35), `filter().enumerate()` chaining (51), partition + tuple destructuring (57), enumerate over struct elements (118). **Real gap found and closed: the existing `t_vecpartition` oracle probes a hand-written generic `split` and NEVER CALLS `Vec.partition`** — the stdlib method was unpinned. 7 oracles banked (492/492). ✅ **for_each/reduce DONE 2026-07-24** — added to std/collections.ax after `fold`. `for_each[T](self: Vec[T], f: fn(T))` (void fn-type param — verified it parses+lowers) applies `f` for side effects; effects flow through a named free fn / module global since lambdas stay zero-capture (BUG#73). The methods NEVER needed capture — the old "blocked" note conflated typical USE with the signature (map/filter/fold are equally zero-capture and shipped fine). `reduce[T](self, f: fn(T,T)->T) -> Option[T]` folds with the FIRST element as init, None on empty (distinct from `fold`'s explicit init). Oracle `t_vecreduce(88)`: named-fn reduce + inline-lambda reduce + empty→None + for_each-into-global, O0–O3 identical. Pure additive stdlib (frontend), A==B trivial. Ghi chú lịch sử (blocked-on-capture): — NOW UNBLOCKED by `2cc67ed` (inline lambda × generic monomorphization works, incl. U≠T & multi-arg — see [[bug-lambda-generic-fntype-infer-open]]). Currently CLEAN-REJECT ("no method 'map' on type Vec"). `std/vec.ax` is the MATH lib (Vec2/3/4); dynamic `Vec[T]` (push/get/index) is compiler-builtin/runtime — need to find where Vec[T] methods live and add generic HOFs returning a new Vec[U]. `Option.map`/`filter` already work (std/result.ax). Moderate feature; good demonstrator of the lambda fix. Watch: element aggregate/16B, U≠T Vec[U] alloc, and the deferred tuple-element-in-generic coercion (§ below) if elements are tuples.
