...
- [✅ None-first generic-inference segfault FIXED (+un-inferable→reject)](bug-none-first-generic-inference.md) — `pick(None, Some(42))` (generic `[T]`, bare None + concrete sibling) mis-monomorphized → segfault; None=Option TEMPLATE poisoned T qua first-wins. Fix1 `4aa4868`: concrete arg override generic binding (oracle t_noneinfer). Fix2 `2438ba0`: un-inferable param trong CONCRETE context (s3/n2) → REJECT sạch qua `current_fn_is_generic` (oracle t_uninferreject). CÒN OPEN: None-VALUE lowering (vG return-None→231, inferable nên ko reject) = session riêng.
