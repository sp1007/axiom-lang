...
- [✅ None-first generic-inference segfault FIXED](bug-none-first-generic-inference.md) — `pick(None, Some(42))` (generic `[T]`, bare None arg + concrete sibling) mis-monomorphized → segfault; None=Option TEMPLATE poisoned T qua first-wins. Fix: concrete arg override generic binding trong `infer_generic_type_args` (A==B `9A2788B6`, oracle t_noneinfer). CÒN OPEN sub-bug: None-VALUE lowering (vG return-None→231; n2 expected-return-only inference→segfault) = session riêng.
