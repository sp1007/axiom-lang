...
- [🔴 OPEN generic-fn float-RETURN → default int miscompile](bug-generic-fn-float-return-default-int.md) — `fn id[T](a:T)->T; id(3.5)` KHÔNG annotate → result type = generic-T → DEFAULT về int → caller chèn `itof` bậy → rác (O0=0/O1=96, divergent). Int instance đúng (default=int), float sai. Workaround: `let r: f64 = id(3.5)` (z2 chạy). Fix ở typecheck ~L3629 result_type=fi.ret (instance signature ret chưa concretize / cần substitute inferred[] trước free L3617). Frontend=A==B. Khác t_f32generic (arg side). Probe batch 6.
