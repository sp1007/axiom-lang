...
**Continuous supervision (NO hibernation):** the loop must be **self-perpetuating** — never end an iteration by hibernating and waiting for the user to type "tiếp tục". At the end of every iteration (commit shipped, RED reverted, or a hard blocker recorded), immediately schedule the next continuation via `ScheduleWakeup(prompt="tiếp tục")` — **never** `ScheduleWakeup(stop:true)** unless the user explicitly says to stop. Cadence: a **~5-minute (300s) supervision heartbeat** as the *fallback* wake; during active work, background-task completions (regression/fixpoint) re-invoke faster, so react to those immediately and treat the 300s wake as a safety net for when nothing else notifies. On each wake: run Phase 0 context check (git status, in-flight work, OPEN bugs), **auto-determine the next task** by the backlog priority order, and execute it — no confirmation. Correctness rules still bind: if the backlog yields only risky/blocked work, do a low-risk task (probing, oracle banking, docs) rather than force a destabilizing change, but keep the loop alive. The user can always interrupt.
